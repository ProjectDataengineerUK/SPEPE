...
| **Version** | 6.0 — Social Module v1.2 completo; **10 agentes** (coletor, explicador, modelista-bayesiano, perfilador, analista_seguranca, contextualizador_saude, sentinela_social, vigilante, analista-eleitoral, narrador); **20 Cloud Run Jobs** (+candidatos_discovery); **18 tabelas Gold** (+dim_territorio derivada de fact_municipio_candidato_eleicao com inline UF→região; +fact_municipio_candidato_eleicao; -fact_candidato_dia; fact_candidato_eleicao renomeada); **22 views semânticas** (+vw_social_candidato_sentimento, vw_social_temas_uf, vw_social_plataforma_uf, vw_social_crise_detector, vw_social_credibilidade, vw_candidato_360, vw_transferencias_candidato, vw_emendas_candidato_uf); **272 testes** (15 arquivos); GDELT re-enabled (controlado por GDELT_ENABLED env); Social NLP: enrich_sentiment_vertex() float + confianca_nlp gate 0.70; IAM: roles/run.viewer adicionado ao Cloud Run SA (Sentinel ExecutionsClient); Terraform: sentinel.tf + pubsub_sentinel.tf DELETADOS (Sentinel integrado ao serviço principal spepe-prod, não Cloud Run separado); Dashboard: Admin button + 27 UFs + applyFilters() real BQ API calls; deploy.yml: GOOGLE_MAPS_API_KEY removido do staging |
...
| 175 | `infra/terraform/sentinel.tf` | ~~Create~~ **DELETADO** | ~~Cloud Run Sentinel separado~~ — Sentinel integrado ao serviço principal spepe-prod; arquivo removido pois declarava Cloud Run inexistente e referenciava pubsub_sentinel.tf | @gcp-data-architect | None |
| 176 | `infra/terraform/pubsub_sentinel.tf` | ~~Create~~ **DELETADO** | ~~Topics sentinel-events/alerts~~ — topics já existem em pubsub.tf; arquivo removido pois causava duplicate resource conflict no Terraform | @gcp-data-architect | 175 |
...
| 5.0 | 2026-05-06 | iterate-agent | **13 módulos — +3 Portal da Transparência**: CadÚnico/Bolsa Família (cadunico_client.py + cadunico_ingest_job, Silver transferencias_sociais, Gold fact_transferencias_sociais) · Emendas Parlamentares (emendas_client.py + emendas_ingest_job, /emendas-parlamentares, análise clientelismo × voto) · CEIS+CNEP Sanções (sancoes_client.py + sancoes_ingest_job, join nm_sancionado × TSE, "eleitores punem ficha suja?") · TRANSPARENCIA_API_KEY wired em todos os clients · Cloud Run Jobs: 12→19 total · deploy.yml + Terraform cloud_run_jobs.tf atualizados |
| 5.1 | 2026-05-07 | iterate-agent | **Correções pós-build**: Gold 8→16 tabelas (nomes corretos: fact_emendas_parlamentar + fact_emendas_municipio, não fact_emendas_parlamentares; fact_sancoes_uf não fact_sancoes_federais; +fact_secao_eleicao, fact_candidato_dia, fact_social_municipio, fact_meta_ads_uf, fact_meta_ads_demografico, fact_google_trends_uf) · 14 views semânticas documentadas (vw_*) · GDELT desabilitado por rate limit severo — social_ingest usa Bluesky (bluesky_client.py) + RSS (news_rss_client.py) · DIEESE: DEFAULT_ANO=2025 (fix Terraform) · deploy.yml: create-or-update para novos jobs · 190 testes (não 172) · 20 clientes (não 12) |
| 6.0 | 2026-05-08 | iterate-agent | **Social Module v1.2 + Infra fixes**: 7→10 agentes (+sentinela_social, analista-eleitoral, contextualizador_saude) · 19→20 Cloud Run Jobs (+candidatos_discovery) · 16→18 Gold tables (+dim_territorio inline UF→região, +fact_municipio_candidato_eleicao) · 14→22 views semânticas (+8 sociais: vw_social_candidato_sentimento, vw_social_temas_uf, vw_social_plataforma_uf, vw_social_crise_detector, vw_social_credibilidade, vw_candidato_360, vw_transferencias_candidato, vw_emendas_candidato_uf) · 190→272 testes · Social NLP: enrich_sentiment_vertex() float + confianca_nlp gate · GDELT re-enabled (GDELT_ENABLED flag) · social_ingest: BQ dinâmico, Instagram, Google Trends movido de digital · candidatos_discovery_job.py + discover_candidate_pages.py · 5 Cloud Schedulers UTC (spepe-social-rss-trends-daily, spepe-social-video-4xday, spepe-social-community-2xday, spepe-social-twitter-daily, spepe-candidatos-discovery-monthly) · Terraform fixes: sentinel.tf + pubsub_sentinel.tf DELETADOS (Sentinel integrado ao serviço principal) · IAM: +roles/run.viewer para Cloud Run SA · deploy.yml: GOOGLE_MAPS_API_KEY removido do staging · Dashboard: Admin button + 27 UFs + applyFilters() real API calls + dim_territorio fix |
